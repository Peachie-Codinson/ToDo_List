...
const ToDoList = () => {
    ...
    const fetchTodos = async () => {
        try {
            ...
            const response = await api.get(`todos/${endpoint}/`);
            if (showCompleted) {
    ...
    const markAsDone = async (id) => {
        try {
            const todo = inProgressTodos.find(todo => todo.id === id);
            const updatedTodo = {
                ...todo,
                status: 'completed',
                    ...
                    <div>
                        ...
                        <div className="row justify-content-center mb-4">
                            <div className="btn-group col-md-8">
                                <Button variant="secondary" onClick={() => sortTodos('date_created')}>
                                    Sort by Date Created
                                </Button>
                                <Button variant="secondary" onClick={() => sortTodos('date_ended')}>
                                    Sort by Deadline
                                </Button>
                                <Button variant="secondary" onClick={() => sortTodos('priority')}>
                                    Sort by Priority
                                </Button>
                            </div>
                        </div>
